...
namespace UnityStandardAssets.Vehicles.Motobike
{
    ...
    public class motorbike_controller : MonoBehaviour
    {
        ...
        [Range(0, 40)] [SerializeField] private float sensitivity = 10;
        ...
        void FixedUpdate()
        {
            ...
            m_Rigidbody = GetComponent<Rigidbody>();
            Vector3 Balancedcom = -3 * Vector3.up;
            ...
            //Lean into corner; removes any balance constraints, resets and offsets com.
            if (Mathf.Abs(m_SteerAngle) > Mathf.Abs(BalanceAngle) && Mathf.Abs(m_SteerAngle) > sensitivity)
            {
                m_Rigidbody.constraints = RigidbodyConstraints.None;
                m_CentreOfMassOffset = transform.right * m_SteerAngle/90/9 + Balancedcom;
            }

            //Balance out of corner; Freezes y rot and -offsets com.
            if (Mathf.Abs(m_SteerAngle) < Mathf.Abs(BalanceAngle) && Mathf.Abs(m_SteerAngle) < sensitivity)
            {
                //m_Rigidbody.constraints = RigidbodyConstraints.FreezeRotationY;
                m_CentreOfMassOffset = transform.right * -BalanceAngle/90/9 + Balancedcom;

            }
            
            if (Mathf.Abs(BalanceAngle) < 2 && Mathf.Abs(m_SteerAngle)<sensitivity)
            {
                //constrains y,z rot 2 degrees from global up (plane normal vector)**NEEDS WORK**
                m_Rigidbody.constraints = RigidbodyConstraints.FreezeRotationY | RigidbodyConstraints.FreezeRotationZ;
                m_CentreOfMassOffset = Balancedcom;
        ...
        private void SteerHelper()
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                if (wheelhit.normal == Vector3.zero)
                    return; // wheels arent on the ground so dont realign the m_Rigidbody velocity
            }
